...
from scripts.generate_capability_boundary_replay_artifact import generate_capability_boundary_replay_artifact

REPO_ROOT = Path(__file__).resolve().parents[1]
ARTIFACT_PATH = REPO_ROOT / "artifacts" / "capability_boundary_replay_results.json"
...
def test_generator_output_matches_committed_artifact(tmp_path: Path) -> None:
    output = tmp_path / "capability_boundary_replay_results.json"
    generate_capability_boundary_replay_artifact(output)
...
def test_determinism_and_sanitization(tmp_path: Path) -> None:
    ...
    generate_capability_boundary_replay_artifact(a_path)
    generate_capability_boundary_replay_artifact(b_path)
...
def test_capability_boundary_evidence_behavior() -> None:
    artifact = _load_json(ARTIFACT_PATH)
    ...
    with_data = [
        ...
        if fixture["capability_boundary"]["original_edge_count"] > 0
        or fixture["capability_boundary"]["replay_edge_count"] > 0
        or fixture["capability_boundary"]["original_node_count"] > 0
        or fixture["capability_boundary"]["replay_node_count"] > 0
    ]

    if with_data:
        assert artifact["global_summary"]["fixtures_with_capability_boundary_data"] > 0
    else:
        assert artifact["global_summary"]["fixtures_with_capability_boundary_data"] == 0

    drift_count = sum(1 for fixture in fixtures if fixture["capability_boundary"]["drift_detected"])
    assert artifact["global_summary"]["fixtures_with_boundary_drift"] == drift_count

    if drift_count > 0:
        assert any(fixture["capability_boundary"]["drift_detected"] for fixture in fixtures)
