use crate::codec::hash::blake3_hex;
use crate::codec::hex::{decode_blake3_hex, encode_hex_32, try_decode_blake3_hex};
use crate::codec::merkle::{verify_proof_hash, Hash, MerkleProof, MerkleTree};
...
pub struct SparkEvidencePacketEnvelope {
    ...
    /// Computed Merkle root over the artifact manifest (from preimage), now visible/usable as optional.
    pub manifest_merkle_root: Option<String>,
    /// Ledger Merkle root (if applicable to this evidence context or provided), now visible/usable as optional.
...
pub fn build_spark_evidence_packet_envelope(
    ...
    // Compute manifest_merkle_root here so the computed root is visible/usable in envelope (was previously discarded with _ inside validate)
    let manifest_hashes = manifest_leaf_hashes(&preimage.artifact_manifest);
    let manifest_merkle_root = if !manifest_hashes.is_empty() {
        let merkle = MerkleTree::from_hashes(manifest_hashes);
        Some(encode_hex_32(&merkle.root()))
    } else {
        None
    };
    ...
    let canonical_hash = blake3_hex(&canonical);
...
pub fn validate_spark_evidence_packet_envelope(
    ...
    let calculated_hash = blake3_hex(&envelope.canonical_json);
    if envelope.canonical_hash != calculated_hash {
        return Err(anyhow::anyhow!("canonical_hash mismatch"));
    }

    // Activate/enforce validation of manifest_merkle_root (if present in envelope); ledger_merkle handled in verify_package_value.
    // Makes the previously-internal computed root usable/validated here in the verify path.
    if let Some(provided) = &envelope.manifest_merkle_root {
        let manifest_hashes = manifest_leaf_hashes(&envelope.preimage.artifact_manifest);
        if !manifest_hashes.is_empty() {
            let merkle = MerkleTree::from_hashes(manifest_hashes);
            let computed = encode_hex_32(&merkle.root());
...
fn validate_pdf_extraction_document(document: &PdfExtractionDocument) -> anyhow::Result<()> {
    ...
    if let Some(hash) = &document.source_sha256 {
        decode_blake3_hex("source_sha256", hash)?;
    }
...
/// Collect only *valid* 64-char hex BLAKE3 leaves (filter; internal package logic tolerates synthetic short hashes in tests).
fn manifest_leaf_hashes(manifest: &[ArtifactManifestEntry]) -> Vec<Hash> {
    manifest
        .iter()
        .filter_map(|entry| entry.sha256.as_ref().and_then(|h| try_decode_blake3_hex(h)))
        .collect()
}

/// Collect only *valid* 64-char hex ledger leaves (filter for test fakes).
fn ledger_entry_hashes(ledger: &serde_json::Value) -> Vec<Hash> {
    ledger
        .as_array()
        .map(|entries| {
            entries
                .iter()
                .filter_map(|entry_val| {
                    entry_val
                        .get("entry_hash")
                        .and_then(|v| v.as_str())
                        .and_then(try_decode_blake3_hex)
                })
                .collect()
        })
        .unwrap_or_default()
...
/// Generate a Merkle proof for the manifest entry at `index` (filtered sha256 leaves only).
pub fn generate_manifest_merkle_proof(
    manifest: &[ArtifactManifestEntry],
    index: usize,
) -> anyhow::Result<MerkleProof> {
    // Strict: reject if any claimed sha256 is not valid 64-hex (prevents silent bad leaves)
    for entry in manifest {
        if let Some(h) = &entry.sha256 {
            if try_decode_blake3_hex(h).is_none() {
                return Err(anyhow::anyhow!(
                    "manifest merkle proof: invalid sha256 hex in entry '{}': {}",
                    entry.path,
                    h
                ));
            }
    ...
    let leaves = manifest_leaf_hashes(manifest);
    if leaves.is_empty() {
        return Err(anyhow::anyhow!(
            "manifest merkle proof: no hashable artifact entries"
        ));
    }
    let tree = MerkleTree::from_hashes(leaves);
    tree.try_generate_proof(index)
}

/// Verify a manifest Merkle proof against an expected root and leaf digest (hex).
pub fn verify_manifest_merkle_proof(
    root_hex: &str,
    leaf_hash_hex: &str,
    proof: &MerkleProof,
) -> bool {
    let Some(expected_root) = try_decode_blake3_hex(root_hex) else {
        return false;
    };
    let Some(leaf) = try_decode_blake3_hex(leaf_hash_hex) else {
        return false;
    };
    if proof.root_hash != expected_root || proof.leaf_hash != leaf {
        return false;
    }
    verify_proof_hash(leaf, proof)
}

/// Generate a Merkle proof for the ledger entry at `index`.
pub fn generate_ledger_merkle_proof(
    ledger: &serde_json::Value,
    index: usize,
) -> anyhow::Result<MerkleProof> {
    // Strict validation for proof gen: every entry_hash must be valid 64-hex
    if let Some(entries) = ledger.as_array() {
        for (i, e) in entries.iter().enumerate() {
            if let Some(h) = e.get("entry_hash").and_then(|v| v.as_str()) {
                if !h.is_empty() && try_decode_blake3_hex(h).is_none() {
                    return Err(anyhow::anyhow!(
                        "ledger merkle proof: invalid entry_hash hex at [{}]: {}",
                        i,
                        h
                    ));
                }
    ...
    let leaves = ledger_entry_hashes(ledger);
    if leaves.is_empty() {
        return Err(anyhow::anyhow!(
            "ledger merkle proof: ledger has no entries"
        ));
    }
    let tree = MerkleTree::from_hashes(leaves);
    tree.try_generate_proof(index)
...
/// Verify a ledger Merkle proof against an expected root and entry_hash (hex).
pub fn verify_ledger_merkle_proof(
    root_hex: &str,
    entry_hash_hex: &str,
    proof: &MerkleProof,
) -> bool {
    ...
    let Some(entry_hash) = try_decode_blake3_hex(entry_hash_hex) else {
        return false;
    };
    if proof.root_hash != expected_root || proof.leaf_hash != entry_hash {
        return false;
    }
    verify_proof_hash(entry_hash, proof)
}
...
fn validate_spark_evidence_preimage(preimage: &SparkEvidencePacketPreimage) -> anyhow::Result<()> {
    ...
    // Merkle computation for manifest_merkle_root (and ledger) now performed in build_spark_evidence_packet_envelope
    // and enforced (if provided) in validate_spark_evidence_packet_envelope so roots are visible/usable on Envelope.
    // (Previously discarded here as _manifest_merkle_root.)
...
pub fn build_package_from_value(value: &serde_json::Value) -> anyhow::Result<serde_json::Value> {
    ...
    let payload_sha256 = blake3_hex(payload_canonical);
    ...
    let final_state_hash = blake3_hex(sidecar_preimage_canonical);
    ...
    let integrity_hash = blake3_hex(package_preimage_canonical);
...
pub fn verify_package_value(value: &serde_json::Value) -> anyhow::Result<()> {
    ...
    let payload_canonical = canonical_json(payload);
    let calculated_payload_sha256 = blake3_hex(payload_canonical);
    ...
    let calculated_final_state_hash = blake3_hex(sidecar_preimage_canonical);
    ...
    let calculated_integrity_hash = blake3_hex(package_preimage_canonical);
    ...
    if let Some(ledger_val) = pkg.get("ledger") {
        ...
        // Merkle-based integrity for ledger (additional to chain, enables efficient proofs)
        let entry_hashes = ledger_entry_hashes(ledger_val);

        if !entry_hashes.is_empty() {
            let merkle = MerkleTree::from_hashes(entry_hashes);
            let computed_merkle_root = encode_hex_32(&merkle.root());
            if let Some(merkle_root) = pkg.get("ledger_merkle_root").and_then(|v| v.as_str()) {
                if merkle_root != computed_merkle_root {
                    return Err(anyhow::Error::new(SparkError::ConstraintDrift(format!(
                        "ledger merkle root mismatch: expected '{}' got '{}'",
                        merkle_root, computed_merkle_root
                    ))));
                }
            }
            // ledger_merkle_root (and manifest_merkle_root) are now visible as optional fields on SparkEvidencePacketEnvelope
            // (computed roots no longer internal/discarded for the evidence packet path)
        }
    }
